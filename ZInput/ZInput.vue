<template>
  ...
  >
    ...
    <input
      ...
      @confirm="onConfirm"
    />
    <div class="u-input__right-icon u-flex">
      <div
        class="u-input__right-icon__clear u-input__right-icon__item"
        @tap="onClear"
        v-if="clearable && value != '' && focused"
      >
        <van-icon size="18" name="close-circle-fill" color="#c0c4cc" />
      </div>
      <div
        class="u-input__right-icon__clear u-input__right-icon__item"
      ...
      >
        <van-icon name="arrow-down-fill" size="26" color="#c0c4cc"></van-icon>
      </div>
    </div>
  </div>
</template>
 ...
 * @tutorial http://udivui.com/components/input.html
 ...
 * @property {String Number} height 高度，单位px(text类型时为70，textarea时为100)
...
export default {
  ...
  props: {
    ...
    placeholderStyle: {
      type: String,
      default: "color: #c0c4cc;padding-left:18px",
    },
    confirmType: {
      type: String,
      default: "done",
  ...
  methods: {
    ...
    handleInput(event) {
      ...
      // 判断是否去除空格
      if (this.trim) value = this.$fn.trim(value);
      // vue 原生的方法 return 出去
      ...
      setTimeout(() => {
        ...
        // #ifdef MP-TOUTIAO
        if (this.$fn.trim(value) == this.lastValue) return;
